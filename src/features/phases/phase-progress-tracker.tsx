...
import type { currentPhaseProgressValidator } from "@/convex/phases/progress"
import { cn } from "@/lib/utils"
import { useQuery } from "convex/react"
import type { Infer } from "convex/values"
import type { ReactNode } from "react"

type PhaseProgressData = Infer<typeof currentPhaseProgressValidator>
...
function usePhaseProgress(owner: CompetitionOrProjectRef) {
  const competitionProgress = useQuery(
    api.competitions.queries.getCurrentPhaseProgress,
    owner.type === "competitions" ? { id: owner.id } : "skip"
  )
  const projectProgress = useQuery(
    api.projects.queries.getCurrentPhaseProgress,
    owner.type === "projects" ? { id: owner.id } : "skip"
  )

  return owner.type === "competitions" ? competitionProgress : projectProgress
}

export function PhaseProgressTrackerForOwner({
  owner,
}: {
  owner: CompetitionOrProjectRef
}) {
  const data = usePhaseProgress(owner)
  return <PhaseProgressTracker data={data} />
}
