...
describe("weekendOverrides behavior", () => {
	test("setOverride creates a new weekend override record", async () => {
		...
		await authed.mutation(api.weekendOverrides.setOverride, {
			satDate: "2026-04-04",
			eventNote: "Test event",
			reserved: true,
		});

		const rows = await authed.query(api.weekendOverrides.list, {});
	...
	test("clearAll deletes all override records (director only)", async () => {
		...
		await volunteer.mutation(api.weekendOverrides.setOverride, {
			satDate: "2026-04-04",
			reserved: true,
		});
		await volunteer.mutation(api.weekendOverrides.setOverride, {
		...
		let rows = await director.query(api.weekendOverrides.list, {});
		expect(rows).toHaveLength(2);

		await director.mutation(api.weekendOverrides.clearAll, {});
		rows = await director.query(api.weekendOverrides.list, {});
	...
	test("list returns all overrides", async () => {
		...
		const rows = await volunteer.query(api.weekendOverrides.list, {});
		expect(rows).toHaveLength(2);
	});
});
