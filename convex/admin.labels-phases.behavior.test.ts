...
describe("labels and phases admin behavior", () => {
	test("create label stores record with name and color", async () => {
		...
		const labelId = await director.mutation(api.labels.create, {
			name: "Bug",
			color: "#ff0000",
		});
	...
	test("update label changes name and color", async () => {
		...
		await director.mutation(api.labels.update, {
			id: labelId,
			name: "Feature",
			color: "#00ff00",
		});
	...
	test("delete label removes the record", async () => {
		...
		await director.mutation(api.labels.remove, { id: labelId });
	...
	test("deleteLabelIfUnused rejects deletion when label is used by a task", async () => {
		...
		await expect(
			director.mutation(api.admin.deleteLabelIfUnused, { id: labelId }),
		).rejects.toBeTruthy();
	});
	...
	test("createPhaseAdmin stores phase record", async () => {
		...
		const phaseId = await director.mutation(api.admin.createPhaseAdmin, {
			key: "planning",
			name: "Planning",
	...
	test("updatePhaseAdmin changes name, description, archived", async () => {
		...
		await director.mutation(api.admin.updatePhaseAdmin, {
			id: phaseId,
			name: "Execution v2",
	...
	test("deletePhaseIfUnused rejects when phase is used by a task", async () => {
		...
		await expect(
			director.mutation(api.admin.deletePhaseIfUnused, { id: phaseId }),
		).rejects.toBeTruthy();
	});
	...
	test("deletePhaseIfUnused succeeds for unused phase", async () => {
		...
		await director.mutation(api.admin.deletePhaseIfUnused, { id: phaseId });
		const doc = await t.run((ctx) => ctx.db.get("phases", phaseId));
		expect(doc).toBeNull();
	});
});
