...
describe("updates CRUD behavior", () => {
	test("create stores competitionUpdates record with correct fields", async () => {
		...
		const updateId = await authed.mutation(api.updates.create, {
			competitionId,
			status: "on-track",
			message: "All good",
		});
	...
	test("create sends progress_update_added notification to subscribers", async () => {
		...
		await authed.mutation(api.updates.create, {
	...
	test("create rejects users without competition access", async () => {
		...
		await expect(
			denied.mutation(api.updates.create, {
				competitionId,
				status: "on-track",
			}),
		).rejects.toBeTruthy();
	...
	test("update changes status and message", async () => {
		...
		await authed.mutation(api.updates.update, {
			updateId,
			status: "off-track",
			message: "Changed",
		});
	...
	test("remove deletes the update record", async () => {
		...
		await authed.mutation(api.updates.remove, { updateId });
