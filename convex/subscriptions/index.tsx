...
const SUBSCRIBABLE_TABLES = [v.literal("competitions"), v.literal("users")] as const;

export const SUBSCRIPTION_TABLE = defineTable(
  v.union(
    ...SUBSCRIBABLE_TABLES.map((tableName) => v.object({
      userId: v.id("users"),
      objectType: v.literal(tableName.value),
      objectId: v.id(tableName.value),
    })),
  ),
).index("by_userId_and_objectType_and_objectId", ["userId", "objectType", "objectId"])

export const getSubscriptionRecord = internalQuery({
  args: {
    userId: v.id("users"),
    objectType: v.union(...SUBSCRIBABLE_TABLES.map((table) => v.literal(table.value))),
