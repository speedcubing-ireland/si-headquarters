...
describe("competitions behavior characterization", () => {
	test("create is forbidden for non-volunteers", async () => {
		...
		await expect(
			authed.mutation(api.competitions.api.create, {
	...
	test("create succeeds for volunteers", async () => {
		...
		const competitionId = await authed.mutation(api.competitions.api.create, {
	...
	test("update is forbidden for users without competition access", async () => {
		...
		await expect(
			denied.action(api.competitions.api.update, {
	...
	test("update clears nullable fields when null values are provided", async () => {
		...
		const updated = await authed.query(api.competitions.api.get, {
			competitionId: seeded.competitionId,
		});
	...
	test("manual sponsor status update requires sponsorship manager permission", async () => {
		...
		await expect(
			authed.action(api.competitions.api.update, {
				competitionId: seeded.competitionId,
	...
	test("manual sponsor status override is applied and can be cleared", async () => {
		...
		const beforeOverride = await authed.query(api.competitions.api.getForUI, {
			competitionId: seeded.competitionId,
		});
		...
		const overridden = await authed.query(api.competitions.api.getForUI, {
			competitionId: seeded.competitionId,
		});
		...
		const afterClear = await authed.query(api.competitions.api.getForUI, {
			competitionId: seeded.competitionId,
		});
