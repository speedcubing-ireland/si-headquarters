...
describe("competition access index behavior", () => {
	test("create and update keep non-volunteer competition visibility in sync", async () => {
		...
		const competitionId = await volunteer.mutation(
			api.competitions.api.create,
			{
				name: "Alice Comp",
				description: "",
		...
		const aliceInitially = await alice.query(
			api.competitions.api.listForUI,
			{},
		);
		const bobInitially = await bob.query(api.competitions.api.listForUI, {});
		expect(
		...
		await alice.action(api.competitions.api.update, {
			competitionId,
			updates: {
		...
		const bobAfterTransfer = await bob.query(
			api.competitions.api.listForUI,
			{},
		);
		expect(
