...
async function seedView(t: ReturnType<typeof convexTest>): Promise<{
	...
	const viewId = await authed.mutation(api.core.views.createView, {
		entity: "tasks",
		pageId: "board",
		name: "My View",
...
describe("saved views behavior", () => {
	...
	test("listViews returns only views for the authenticated user, entity, and pageId", async () => {
		...
		await otherAuthed.mutation(api.core.views.createView, {
			entity: "tasks",
			pageId: "board",
			name: "Other View",
			filtersJson: "{}",
		...
		await authed.mutation(api.core.views.createView, {
			entity: "competitions",
			pageId: "board",
			name: "Comp View",
			filtersJson: "{}",
		...
		const rows = await authed.query(api.core.views.listViews, {
			entity: "tasks",
			pageId: "board",
		});
	...
	test("touchView updates lastUsedAt without changing updatedAt", async () => {
		...
		await authed.mutation(api.core.views.touchView, { id: viewId });
	...
	test("updateView changes name and description, advances updatedAt", async () => {
		...
		const before = await t.run((ctx) => ctx.db.get("savedViews", viewId));
		await authed.mutation(api.core.views.updateView, {
			id: viewId,
			name: "Renamed",
	...
	test("updateView throws FORBIDDEN for another user's view", async () => {
		...
		await expect(
			otherAuthed.mutation(api.core.views.updateView, {
				id: viewId,
				name: "Hijack",
			}),
		).rejects.toBeTruthy();
	...
	test("deleteView removes the record", async () => {
		...
		await authed.mutation(api.core.views.deleteView, { id: viewId });
		const doc = await t.run((ctx) => ctx.db.get("savedViews", viewId));
		expect(doc).toBeNull();
	});
	...
	test("deleteView throws FORBIDDEN for another user's view", async () => {
		...
		await expect(
			otherAuthed.mutation(api.core.views.deleteView, { id: viewId }),
		).rejects.toBeTruthy();
	});
	...
	test("touchView throws FORBIDDEN for another user's view", async () => {
		...
		await expect(
			otherAuthed.mutation(api.core.views.touchView, { id: viewId }),
		).rejects.toBeTruthy();
	});
});
