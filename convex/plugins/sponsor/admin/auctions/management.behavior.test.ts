...
describe("auction management behavior", () => {
	...
	test("update changes framework and dates in draft state", async () => {
		...
		await manager.mutation(api.plugins.sponsor.admin.auctions.management.update, {
			auctionId,
			framework: "vickrey",
			startsAt: newStart,
			endsAt: newEnd,
	...
	test("removeBeforeOpen rejects active auctions", async () => {
		...
		await expect(
			manager.mutation(api.plugins.sponsor.admin.auctions.management.removeBeforeOpen, {
				auctionId,
			}),
		).rejects.toBeTruthy();
	});
