...
describe("task integrations", () => {
  ...
  test("rejects starting a run while integration is already running", async () => {
    ...
    await expect(
      t
        ...
        .mutation(api.integrations.taskIntegrations.mutations.run, {
          id: integrationRowId,
        })
  ...
  test("attaches configured template integrations when tasks are created", async () => {
    ...
    const listed = await t
      .withIdentity({ subject: userId })
      .query(api.integrations.taskIntegrations.queries.listForTask, { taskId })
    expect(listed.map((row) => row.integrationId)).toEqual([
      "canva.certificates",
    ])
  })

  test("project tasks cannot attach competition integrations", async () => {
    const t = convexTest(schema, modules)
    const { userId, taskId } = await t.run(async (ctx) => {
      ...
      await ctx.db.insert("projectMembers", {
        projectId,
        member: { type: "users", id: userId },
      })
      const phaseId = await insertProjectPhase(ctx, projectId, "Ops", "a")
      const taskId = await insertSeedTask(ctx, {
        name: "Project task",
        parent: { type: "phases", id: phaseId },
        order: "a",
    ...
    const asVolunteer = t.withIdentity({ subject: userId })

    await expect(
      asVolunteer.mutation(api.integrations.taskIntegrations.mutations.attach, {
        taskId,
        integrationId: "canva.certificates",
      })
    ).rejects.toThrow(/competition-scoped task/)

    const available = await asVolunteer.query(
      api.integrations.taskIntegrations.queries.listAvailableForTask,
      { taskId }
    )
    expect(available).toEqual([])

    const listed = await asVolunteer.query(
      api.integrations.taskIntegrations.queries.listForTask,
      { taskId }
    )
