...
export const linkTaskCanvaDesign = action({
	...
	handler: async (ctx, args): Promise<RunTaskLinkedActionResult> => {
		...
		const canRun = await ctx.runQuery(
			internal.integrations.linkedActions.canUserRunTaskLinkedAction,
			{
				taskId: args.taskId,
				taskLinkedActionId: args.taskLinkedActionId,
		...
		try {
			const design = await ctx.runAction(
			...
			await ctx.runMutation(
				internal.integrations.linkedActions.setTaskLinkedActionRunState,
				{
...
export const completeLinkedSheetShareWithLaptops = action({
	...
	handler: async (ctx, args): Promise<RunTaskLinkedActionResult> => {
		...
		const linkedActions = await ctx.runQuery(
		...
		const shareResult = await ctx.runAction(
			api.integrations.wca.schedule.shareSheetWithLaptops,
			{
				competitionId,
			},
		);
