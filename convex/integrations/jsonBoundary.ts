...
function narrowJsonRoot(value: JsonWireValue): JsonRoot {
  if (Array.isArray(value)) {
    ...
    for (const entry of value) {
      // oxlint-disable-next-line typescript/no-unsafe-argument -- JSON arrays are narrowed item-by-item at this boundary.
...
function parseJsonText(text: string): JsonRoot {
  // oxlint-disable-next-line typescript/no-unsafe-argument -- JSON.parse is narrowed before callers can read from it.
...
export async function readJsonObjectArray(
  ...
  for (const entry of body) {
    ...
    // oxlint-disable-next-line typescript/no-unsafe-argument -- Array entries are checked as plain objects before exposing them.
