...
describe("teams behavior", () => {
	test("create team stores record with name and member IDs", async () => {
		...
		const teamId = await director.mutation(api.teams.create, {
			name: "New Team",
			memberIds: [memberId],
		});
	...
	test("update team changes name and memberIds", async () => {
		...
		await director.mutation(api.teams.update, {
			teamId,
			name: "Renamed",
			memberIds: [memberId],
		});
	...
	test("list returns all teams", async () => {
		...
		await director.mutation(api.teams.create, { name: "Alpha" });
	...
	test("get returns a single team by ID", async () => {
		...
		const doc = await director.query(api.teams.get, { teamId });
	...
	test("create rejects non-directors", async () => {
		...
		await expect(
			authed.mutation(api.teams.create, { name: "Should fail" }),
		).rejects.toBeTruthy();
	});
});
