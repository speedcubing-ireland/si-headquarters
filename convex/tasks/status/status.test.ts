...
describe("Regression coverage", () => {
  ...
  describe("2. Phase Making Active regressions", () => {
    test("activating a phase moves direct backlog tasks to to-do", async () => {
      ...
      await user.mutation(api.tasks.mutations.activatePhaseTasks, { phaseId })
      const taskStatuses = await t.run(async (ctx) => {
      ...
      expect(taskStatuses).toEqual(["to-do", "in-progress"])
    })
    ...
    test("activating a phase recursively activates standard backlog subtasks", async () => {
      ...
      const { phaseId, taskId, childId, grandchildId } = await t.run(
        async (ctx) => {
          ...
          return { phaseId, taskId, childId, grandchildId }
        }
      )
    ...
    test("activating a phase activates the current step of direct backlog flows", async () => {
      ...
      expect(flow.parentStatusView.effectiveStatus).toBe("to-do")
      expect(
        flow.subtasks.map(({ statusView }) => ({
          status: statusView.effectiveStatus,
        }))
      ).toEqual([{ status: "to-do" }, { status: "backlog" }])
    })
    ...
    test("activating a phase activates direct subtasks of a flow current step", async () => {
      ...
      const { phaseId, currentStepId, childId, futureStepId } = await t.run(
        async (ctx) => {
          ...
          return { phaseId, currentStepId, childId, futureStepId }
        }
      )
    ...
    test("activating a phase lets nested flow subtasks manage their own current step", async () => {
      ...
      const {
        ...
        const flowSubtaskId = await insertTask(ctx, {
      ...
      expect(parent.effectiveStatus).toBe("to-do")
      expect(intentCommand(flow.parentStatusView.statusIntent)).toBe("auto")
      expect(flow.parentStatusView.flow?.currentStepId).toBe(firstStepId)
      expect(
        flow.subtasks.map(({ task, statusView }) => ({
