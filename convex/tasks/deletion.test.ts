...
import { deleteEntitySubscriptions } from "../notifications/lib/cleanup";
...
describe("deleteEntitySubscriptions", () => {
	test("deletes only matching entity subscriptions", async () => {
		...
		expect(
			rows.some(
				(row) => row.entityType === "task" && row.entityId === "task-1",
			),
		).toBe(false);
		expect(
			rows.some(
				(row) => row.entityType === "task" && row.entityId === "task-2",
			),
		).toBe(true);
		expect(
			rows.some(
				(row) => row.entityType === "competition" && row.entityId === "comp-1",
			),
		).toBe(true);
		expect(
			rows.some(
