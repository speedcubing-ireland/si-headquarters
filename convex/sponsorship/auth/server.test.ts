...
describe("isSponsorPasswordAuthEnabled", () => {
  ...
  test.each(["1", "true", "yes", "TRUE", "Yes", " true "])(
    "returns true for %s",
    (value) => {
...
describe("buildSponsorOtpEmail sender address", () => {
  ...
  test("falls back to sponsorship@speedcubingireland.com when env unset", async () => {
    ...
    expect(result.from).toBe("sponsorship@speedcubingireland.com")
  })
...
describe("buildSponsorOtpEmail", () => {
  ...
  test("preserves original email as recipient", async () => {
    const result = await buildSponsorOtpEmail({
      email: "Test@Example.COM",
      otp: "123456",
      type: "sign-in",
  ...
  test("includes purpose-specific text for each type", async () => {
    ...
    expect(forgotPassword.text).toContain("reset your password")
    expect(emailVerification.text).toContain("verify your email")
  })
})
