...
import { findAuctionInvite, listAuctionInvites } from "../lib/auctionQueries"
import {
...
export async function replaceAuctionInvites(
  ...
  const current = await listAuctionInvites(ctx, args.auctionId)
...
export async function requireAuctionInviteForSponsor(
  ...
  const invite = await findAuctionInvite(ctx, auctionId, sponsorId)
  if (!invite) {
    throw new ConvexError({
      code: "FORBIDDEN",
