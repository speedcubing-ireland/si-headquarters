...
describe("sendAuctionScheduledEmails", () => {
	...
	test("batch key contains auction ID and auction_scheduled type", async () => {
		const auction = makeAuction();
		const competition = makeCompetition();
		const { ctx, scheduledCalls } = createEmailCtx({
			competition,
		...
		const args = scheduledCalls[0].args as { batchKey: string };
		expect(args.batchKey).toContain(String(auction._id));
		expect(args.batchKey).toContain("auction_scheduled");
	});
});
