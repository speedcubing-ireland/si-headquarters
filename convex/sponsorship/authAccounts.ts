...
function parseSponsorAuthUserDoc(value: unknown): SponsorAuthUserDoc | null {
...
export async function findSponsorAuthUserById(
	...
	const result = await ctx.runQuery(components.sponsorAuth.adapter.findOne, {
		model: "user",
		where: [{ field: "_id", value: authUserId }],
	});
	return parseSponsorAuthUserDoc(result);
}
...
export async function ensureSponsorAuthAccount(
	...
	const existingByOriginalEmail =
		existingByCanonicalEmail || args.sponsor.email === canonicalEmail
			? null
...
export async function requireSponsorByAuthSessionToken(
	...
	const rawSession = await ctx.runQuery(
