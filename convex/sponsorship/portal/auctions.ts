...
async function requireSponsorAuctionForBid(
  ctx: MutationCtx,
  args: { sessionToken: string; auctionId: Id<"sponsorshipAuctions"> }
): Promise<{
  auction: Doc<"sponsorshipAuctions">
  sponsorId: Id<"sponsors">
}> {
  ...
  return { auction, sponsorId: sponsor._id }
}

async function finishSponsorBidMutation(
  ctx: MutationCtx,
  auction: Doc<"sponsorshipAuctions">,
  result: {
    currentPriceCents: number
...
export async function placeBidHandler(
  ...
  const { auction, sponsorId } = await requireSponsorAuctionForBid(ctx, args)
  const result = await placeSponsorshipBid(ctx, {
    auction,
    sponsorId,
    amountCents: args.amountCents,
  })
  return finishSponsorBidMutation(ctx, auction, result)
}
