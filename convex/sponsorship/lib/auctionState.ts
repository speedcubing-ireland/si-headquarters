...
import {
  ...
  type SealedBidIntent,
} from "./bidding"
import { isProxyAuctionFramework, sealedAuctionPricingRule } from "./validators"
...
export function buildSealedBidIntents(
  intents: IntentDoc[]
): SealedBidIntent<string, string>[] {
...
function resolveSealedAuctionOutcome(args: {
  auction: AuctionDoc
  validIntents: IntentDoc[]
}) {
  return resolveSealedOutcome(buildSealedBidIntents(args.validIntents), {
...
> {
  ...
  if (!isProxyAuctionFramework(args.auction.framework)) {
    const sealedState = resolveSealedAuctionOutcome(args)
    if (!sealedState) {
      return {
        currentPriceCents: undefined,
        currentLeaderSponsorId: undefined,
