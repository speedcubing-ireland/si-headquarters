		...
		db: {
			insert: async (table: string, value: Record<string, unknown>) => {
				if (table === "sponsorshipBidIntents") {
					...
					const doc = {
						...value,
						_id: `intent-${insertedIntentCount}` as BidIntentDoc["_id"],
						_creationTime:
				...
				if (table === "sponsorshipBidEvents") {
					...
					const doc = {
						...value,
						_id: `event-${insertedEventCount}` as BidEventDoc["_id"],
						_creationTime:
							typeof value.createdAt === "number"
								? value.createdAt
								: Date.now(),
...
function withPatchedAuction(
	...
	const latestPatch = patches[patches.length - 1]?.patch;
	if (!latestPatch) return auction;
	return {
		...auction,
		currentPriceCents:
...
describe("sponsorship bid placement", () => {
	...
	test("example 1: manual challenger does not expose intermediate bid amount", async () => {
		...
		expect(secondEvents.some((event) => event.amountCents === 1100)).toBe(
			false,
		);
	});
